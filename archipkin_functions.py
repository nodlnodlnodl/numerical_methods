...
def about_norm_choose():
    ...
    Принято выбирать норму в $U_h$ так, чтобы при $h \\rightarrow 0$ она переходила в какую-либо норму функций, заданных на всем отрезке по $x$:
    """)
    ...
    st.latex(r"\|[y]_h - y^{(h)}\|_{U_h^1} = \left( h \sum_m \left( [y]_h - y^{(h)} \right)^2 \right)^{1/2} ")
    ...
    st.latex(
        r"\|Z^{(h)}\|_{U_h} = \max \left[ \max_m |Z^{(h)}_m|, \max_m \frac{1}{h} |Z^{(h)}_{m+1} - Z^{(h)}_m| \right] \tag{22}")

    st.markdown("""
    Для этой нормы условие (17) выполнено, если за $U$ принять пространство непрерывно дифференцируемых функций с нормой
    """)

    st.latex(r"\|y\|_U = \max_{a \leq x \leq b} \left[ \max |y(x)|, \max |y'(x)| \right]")
