...
def polynomial_approximation_difficulties():
    ...
    st.markdown("""
    Согласно выражению для ошибки приближения многочленом (формула 9), ошибку можно выразить через производную аппроксимируемой функции.
    Обычно предполагается, что для больших n эта ошибка мала, но на практике это не всегда так. Рассмотрим функцию:
    """)
    ...
    st.markdown("""
    Для этой функции можно записать следующую производную:
    """)
    ...
    st.markdown("""
    С увеличением n для фиксированного x производные начинают значительно возрастать. Это распространённый случай для многих функций, 
    у которых производные высокого порядка имеют тенденцию резко увеличиваться при росте n . 
    Это приводит к тому, что на некоторых интервалах аппроксимируемая функция перестает сходиться к настоящему значению.
    ...
    st.markdown("""
    Этот пример интерполяции, предложенный Рунге, показывает, что на отрезке [-1, 1], если узлы равномерно распределены, 
    ошибка аппроксимации будет расти с увеличением порядка интерполяции n, особенно на концах отрезка. 
    Данный эффект приводит к значительным отклонениям в аппроксимируемой функции, несмотря на точное совпадение с узловыми значениями.
    """)

    st.markdown("""
    Важно отметить, что интерполяция многочленами на равномерно распределённых узлах может стать причиной роста ошибок, 
    особенно для функций с высокими производными на краях интервала. Это известно как **феномен Рунге**.
    """)
...
def chebyshev_polynomials():
    st.header("3.2.5. Многочлены Чебышёва")

    st.markdown("""
    Многочлены Чебышёва играют важную роль в численном анализе, и их свойства широко используются в вычислительной математике.
    Они определяются на отрезке [-1, 1]  и минимизируют максимальное отклонение на этом интервале, что делает их оптимальными для аппроксимации.

    Многочлены Чебышёва Tn(x) определяются для |x| ≤ 1 с помощью следующего соотношения:
    """)
    ...
    st.markdown("""
    Это свойство позволяет выражать многочлены через косинус, что упрощает их вычисление. Многочлены Чебышёва также могут быть определены через рекуррентные соотношения.

    Многочлены Tn(x) ортогональны на отрезке [-1, 1] и удовлетворяют следующему соотношению:
    """)
    ...
    st.markdown("""
    Многочлены Чебышёва могут быть выражены с помощью следующего рекуррентного соотношения:
    """)
    ...
    st.markdown("""
    Таким образом, многочлены Чебышёва могут быть эффективно вычислены для любого n с помощью рекуррентного соотношения. 
    Эти многочлены минимизируют максимальное отклонение между аппроксимируемой функцией и многочленом на отрезке [-1, 1]ёёё.
    Это свойство делает их идеальными для задач приближения и интерполяции.
    """)