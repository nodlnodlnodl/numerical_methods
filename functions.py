...
def polynomial_approximation_difficulties():
    ...
    st.markdown("""    Согласно выражению для ошибки приближения многочленом (формула 9), ошибку можно выразить через производную аппроксимируемой функции.    Обычно предполагается, что для больших n эта ошибка мала, но на практике это не всегда так. Рассмотрим функцию:    """)
...
def chebyshev_polynomials():
    ...
    st.markdown("""    Таким образом, многочлены Чебышёва могут быть эффективно вычислены для любого n с помощью рекуррентного соотношения.     Эти многочлены минимизируют максимальное отклонение между аппроксимируемой функцией и многочленом на отрезке [-1, 1].    Это свойство делает их идеальными для задач приближения и интерполяции.    """)
