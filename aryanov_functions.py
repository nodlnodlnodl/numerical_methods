...
""")
    ...
    и предположим, что эта система уравнений имеет единственное решение <i>y^{(h)}</i>. Как правило, разностная схема не
    удовлетворяет дифференциальной задаче (1) в точности, т.е. <i>L_h [y] \neq f^{(h)}</i>, и на самом деле она аппроксимирует
...
""")
    ...
    st.markdown("""
    Величина <i>\(\delta f^{(h)}\)</i> будет зависеть от выбора разностной схемы, величины шага \( h \) и точности задачи (1).
    Чтобы величина невязки была как можно меньше, разностная схема должна хорошо аппроксимировать дифференциальную задачу.
    Величина <i>\(\delta f^{(h)}\)</i> зависит от некоторых параметров, таких как размер шага сетки, гладкость функций и другие параметры.
