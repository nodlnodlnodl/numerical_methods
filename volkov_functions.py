...
def runge_error_estimation_extended():
    st.header("11.2.1. Оценка погрешности по правилу Рунге")
    ...
    st.markdown("""
        При таком способе оценки локальной погрешности метод Рунге–Кутты на шаге используется три раза и требует $$3q - 
        1$$ вычислений правой части ОДУ. 
...
def combined_order_error_estimation():
    st.header("11.2.2. Оценка погрешности на основе комбинации формул разных порядков точности")

    st.markdown("""
    Этот способ основан также на использовании двух приближённых решений в одной точке, но в отличие от способа Рунге 
    они получены по формулам разных порядков точности $$P$$ и $$s$$ с одинаковым шагом $$h$$. Сначала рассмотрим общий 
    случай.

    Пусть имеются два метода типа Рунге–Кутты:
    """)
    st.latex(r"""
    y_1^P = y_0 + \sum_{i=1}^r P_i k_i(h), \tag{23}
    """)
    st.markdown("""
    где
    """)
    st.latex(r"""
    k_1 = h f(x_0, y_0); \quad k_i = h f\left(x_0 + \alpha_i h,\  y_0 + \sum_{j=1}^{i-1} \beta_{ij} k_j \right), 
    """)
    st.markdown("""
    и
    """)
    st.latex(r"""
    y_1^s = y_0 + \sum_{i=1}^{\tilde{r}} \tilde{P_i} \tilde{k_i}, \tag{24}
    """)
    st.markdown("""
    где
    """)
    st.latex(r"""
    \tilde{k}_1 = k_1; \  \tilde{k}_i = h f\left(x_0 + \tilde{\alpha}_i h,\  y_0 + \sum_{j=1}^{i-1} \tilde{\beta}_{ij}
    \tilde{k}_j \right).
    """)

    st.markdown("""
    Пусть $$P > s$$ и $$r \\geq \\tilde{r}$$. Локальные погрешности для этих методов имеют вид:
    """)
    st.latex(r"""
    \rho_1^P = y(x_0 + h) - y_1^P = O(h^{P+1}); 
    """)
    st.latex(r"""
    \rho_1^s = y(x_0 + h) - y_1^s = O(h^{s+1}).\quad 
    """)
    st.markdown("""
    Из этих оценок следует, что:
    """)
    st.latex(r"""
    \rho_1^s \equiv y_1^P - y_1^s.
    """)

    st.markdown("""
    Получение оценки $$\\rho_1^s$$ требует $$r + \\tilde{r} - 1$$ вычислений правых частей уравнения $$(14)$$.
    """)

    st.markdown("""
    Теперь рассмотрим специальные случаи. Пусть в формулах $$(23)$$, $$(24)$$ коэффициенты подобраны так, что:
    """)
    st.latex(r"""
    \alpha_i = \tilde{\alpha}_i; \quad \beta_{ij} = \tilde{\beta}_{ij}, \quad i = 1, 2, \dots, \tilde{r}.
    """)
    st.markdown("""
    Тогда $$k_i = \\tilde{k}_i, \\ i = 1, 2, \\dots, \\tilde{r}$$, и для погрешности имеем:
    """)
    st.latex(r"""
    \rho_1^s = y_1^P - y_1^s = \sum_{i=1}^r q_i k_i, \tag{25}
    """)
    st.markdown("""
    где:
    """)
    st.latex(r"""
    q_i = P_i - \tilde{P}_i, \quad i = 1, 2, \dots, \tilde{r}; \quad q_i = P_i, \quad i = \tilde{r} + 1, \dots, r. 
    \tag{26}
    """)
    st.markdown("""
    Такой подход требует только $$r$$ вычислений правых частей. В качестве приближённого решения принимается 
    $$y_1^P$$ как более точное. Величина:
    """)
    st.latex(r"""
    E = \sum_{i=1}^r q_i k_i, \tag{27}
    """)
    st.markdown("""
    где $$q_i$$ определены согласно $$(26)$$, называется **контрольным членом**. В случае системы контрольный член 
    записывается отдельно для каждой компоненты:
    """)
    st.latex(r"""
    E^{(j)} = \sum_{i=1}^r q_i k_i^{(j)}, \quad j = 1, 2, \dots, M.
    """)

    st.markdown("""
    Приведем несколько контрольных членов для методов Рунге–Кутты:

    $$1.$$ Рассмотрим двухчленную формулу $$(8)$$ и трёхчленную $$(12)$$:
    ...
    """)
    st.latex(r"""
    \tilde{k}_1 = h f(x_0, y_0); \tag{28}
    """)
    st.latex(r"""
    \tilde{k}_2 = h f\left(x_0 + \frac{1}{2} h,\  y_0 + \frac{1}{2} \tilde{k}_1\right). 
    """)
    ...
    """)
    st.latex(r"""
    y_1 = y_0 + \frac{1}{6} (\tilde{k}_1 + 4 \tilde{k}_2 + \tilde{k}_3);
    """)
    st.latex(r"""
    \tilde{k}_1 = h f(x_0, y_0);
    """)

    st.latex(r"""
    \tilde{k}_2 = h f\left(x_0 + \frac{1}{2} h, y_0 + \frac{1}{2} \tilde{k}_1\right);\tag{29}
    """)
    st.latex(r"""
    \tilde{k}_3 = h f\left(x_0 + h, y_0 + 2 \tilde{k}_2 - \tilde{k}_1\right). 
    """)

    st.markdown("""
    Здесь $$\\tilde{r} = 2, \\ r = 3$$; видно, что $$k_i = \\tilde{k}_i, \\ \\tilde{k}_2 = k_2$$. Контрольный 
    член имеет вид:
    """)

    st.latex(r"""
    E = \frac{1}{6} (k_1 - 2k_2 + k_3), \tag{30}
    """)
    st.markdown("""
    и имеет порядок $$O(h^3) \\ [P = 3, \\ s = 2].$$

    Если бы мы использовали правило Рунге для метода $$(28)$$, то понадобилось бы пять вычислений правой части вместо 
    трёх в $$(30)$$.

    $$2.$$ Рассмотрим классический четырёхчленный метод Рунге–Кутты:
    """)
    st.latex(r"""
    ...
    """)
    st.latex(r"""
    k_2 = h f\left(x_0 + \frac{1}{2}h,\ \  y_0 + \frac{1}{2}k_1\right);\tag{31}
    """)
    st.latex(r"""
    ...
    st.markdown("""
    и двухчленный метод $$(28)$$. Видно, что $$k_1 = \\tilde{k}_1, \\ k_2 = \\tilde{k}_2$$. Здесь $$P = 4, \\ s = 2, 
    \\ r = 4, \\ \\tilde{r} = 2$$. Контрольный член имеет вид:
    """)
    st.latex(r"""
    E = \frac{1}{6} (k_1 - 4k_2 + 2k_3 + k_4),\tag{32}
    """)
    st.markdown("""
    и имеет порядок $$O(h^3).$$

    3. Мерсон предложил следующую модификацию классического метода Рунге–Кутты:
    """)
    st.latex(r"""
    y_1 = y_0 + \frac{1}{6} (k_1 + 4k_4 + k_5);
    """)
    st.latex(r"""
    k_1 = h f(x_0, y_0);
    """)
    ...
    """)
    st.latex(r"""
    k_3 = h f\left(x_0 + \frac{1}{3}h,\ \  y_0 + \frac{1}{6}k_1 + \frac{1}{6}k_2\right);\tag{33}
    """)
    st.latex(r"""
    k_4 = h f\left(x_0 + \frac{1}{2}h,\ \ y_0 + \frac{1}{8}k_1 + \frac{3}{8}k_3\right);
    """)
    st.latex(r"""
    k_5 = h f\left(x_0 + h,\ \ y_0 + \frac{1}{2}k_1 - \frac{3}{2}k_3 + 2k_4\right). 
    """)

    st.markdown("""
    Эта схема имеет четвёртый порядок точности. Наряду с ней можно рассмотреть схему третьего порядка точности:
    """)
    st.latex(r"""
    y_1 = y_0 + \frac{1}{10} (k_1 + 3k_3 + 4k_4 + 2k_5). \tag{34}
    """)
    st.markdown("""
    Здесь $$r = 5, \\ \\tilde{r} = 5.$$ Контрольный член имеет вид:
    """)
    st.latex(r"""
    E = \frac{1}{30} (2k_1 - 9k_3 + 8k_4 - k_5),\tag{35}
    """)
    st.markdown("""
    и имеет порядок $$O(h^4).$$
    """)

    st.markdown("""
    Если бы контроль проводился по правилу Рунге для метода третьего порядка $$(34)$$, то это потребовало бы восьми
    обращений к вычислительной правой части вместо пяти в методе Мерсона. Существуют и другие фомы контрольный кленов, 
    в том числе и нелинейные.
    """)


# 11.2.3. Интегрирование с переменным шагом. Автоматический выбор шага интегрирования
def adaptive_step_integration():
    st.header("11.2.3. Интегрирование с переменным шагом. Автоматический выбор шага интегрирования")

    st.markdown("""
    Имея в распоряжении способы оценки локальной погрешности (метод Рунге либо контрольные члены), мы можем в 
    процессе счёта менять шаг интегрирования $$h$$ с тем, чтобы, сохраняя необходимую точность, уменьшить число 
    шагов и снизить объём вычислительной работы. При этом обычно исходят из того, чтобы на каждый шаг приходилась 
    примерно одинаковая погрешность (аналогия с обратной задачей теории погрешности!).

    Наиболее простой и распространённый алгоритм выбора шага заключается в следующем. Пусть $$\\rho_{n+1}$$ – 
    оценка локальной погрешности метода на шаге $$h$$, допущенной при вычислении $$y_{n+1}^{(h)}$$ в точке $$x_n + 
    h$$. Если оценка превосходит заданную заранее границу $$\\varepsilon$$:
    """)
    st.latex(r"""
    |\rho_{n+1}| > \varepsilon,
    """)
    st.markdown("""
    то $$y_{n+1}^{(h)}$$ считается неудовлетворяющим предписанной точности и шаг $$h$$ объявляется неприемлемым; 
    точка $$(x_n + h, y_{n+1}^{(h)})$$ исключается из рассмотрения. Выбирается новое значение шага $$h^{(1)} = 
    h / 2$$, и по методу Рунге–Кутты с шагом $$h^{(1)}$$ вычисляется $$y_{n+1}^{(h^{(1)})}$$ в точке $$x_n + 
    h^{(1)}$$. 

    Если снова $$\\rho_{n+1}$$ – оценка локальной погрешности на шаге $$h^{(1)}$$ не удовлетворяет предписанной 
    точности, то шаг $$h^{(1)}$$ вновь делится пополам, и все вычисления повторяются. Так происходит до тех пор, 
    пока при некотором шаге (назовём его $$h_n$$) не будет выполнено неравенство:
    """)
    st.latex(r"""
    |\rho_{n+1}| \leq \varepsilon.
    """)

    st.markdown("""
    После этого считается, что получено значение решения $$y_{n+1}$$ в точке $$x_{n+1} = x_n + h_n$$, и дальнейшее 
    интегрирование происходит от точки $$x_{n+1}$$. 

    Если на некотором шаге $$h_n = x_{n+1} - x_n$$, получается, что:
    """)
    st.latex(r"""
    |\rho_{n+1}| \leq \varepsilon / k,
    """)

    st.markdown("""
    где $$k > 1$$ – некоторая заданная константа, то считается, что точность превышает заданную, и следующий шаг 
    полагается равным $$h_{n+1} = 2h_n$$ (при вычислении решения $$y_{n+2}$$). 

    Если выполняется неравенство:
    """)
    st.latex(r"""
    \frac{\varepsilon}{k} \leq |\rho_{n+1}| \leq \varepsilon,
    """)
    st.markdown("""
    то изменение шага интегрирования не производится: $$h_{n+1} = h_n$$. Константа $$k$$ обычно полагается равной 
    $$2^m$$, где $$m$$ – порядок используемой оценки локальной погрешности метода (порядок контрольного члена). 
    Так, для контрольных членов $$(30)$$ $$k = 8$$, для $$(32)\\ $$ $$k = 8$$, для $$(35)\\ $$ $$k = 16$$. В случае 
    использования правила Рунге для метода Рунге–Кутты порядка $$s\\ $$ $$k = 2^{s+1}$$.

    Чтобы сократить число неприемлемых шагов, используется следующее правило: если при переходе от $$x_n$$ к $$x_{n+1} =
    x_n + h_n$$ шаг интегрирования сокращался хотя бы один раз, при выборе $$h_{n+1}$$ удвоение $$h_n$$ не 
    производится, даже если и $$|\\rho_{n+1}| \\leq \\varepsilon / k.$$
    """)

    st.markdown("""
    Можно рассмотреть другой алгоритм выбора шага, основанный на явном представлении главного члена локальной 
    погрешности метода Рунге-Кутты порядка s
    """)
    st.latex(r"""
    \rho_{n+1} = \psi(x_n, y_n) h^{s+1}.
    """)
    st.markdown("""
    Пусть на некотором шаге $$h$$ полученная каким-либо способом оценка $$\\rho_{n+1}$$ переходит заданную границу 
    $$\\varepsilon$$:
    """)
    st.latex(r"""
    |\rho_{n+1}| > \varepsilon.
    """)
    st.markdown("""
    Тогда полученное значение $$y_{n+1}$$ и точка $$x = x_n + h$$ исключаются из рассмотрения, и производится новый 
    выбор шага $$h$$ по следующему правилу. Введем $$h_{\\varepsilon} = \\alpha h$$ и выберем $$\\alpha$$ из условия:
    """)
    st.latex(r"""
    |\psi(x_n, y_n) h_{\varepsilon}^{s+1}| = \varepsilon.
    """)
    st.markdown("""
    Так как:
    """)
    st.latex(r"""
    \psi(x_n, y_n) = |\rho_{n+1}| / h^{s+1},
    """)
    st.markdown("""
    то:
    """)
    st.latex(r"""
    \alpha^{s+1} = \varepsilon / |\rho_{n+1}|,
    """)
    st.markdown("""
    и:
    """)
    st.latex(r"""
    \alpha = \sqrt[\footnotesize{s+1}]{\varepsilon / |\rho_{n+1}|} < 1.
    """)

    st.markdown("Новое значение шага")
    st.latex(r"""
        h_{\varepsilon} = \sqrt[\footnotesize{s+1}] {\varepsilon / |\rho_{n+1}|} h. \tag{36}
        """)

    st.markdown("""
    По формуле Рунге–Кутты выполняются вычисления с шагом $$h_{\\varepsilon}$$ и находится значение 
    $$y_{n+1}^{(h_{\\varepsilon})}$$ при $$x_n + h_{\\varepsilon}$$. Если же $$|\\rho_{n+1}| < \\varepsilon$$, 
    то $$y_{n+1}$$ считается удовлетворяющим по точности значением решения в точке $$x_{n+1} = x_n + h$$; эта точка
    засчитывается. Дальнейшее интегрирование делается с шагом $$(36)$$, где $$\\alpha \\geq 1$$ и $$h_{\\varepsilon} 
    \\geq  h$$. Этот алгоритм более гибкий по сравнению с предыдущим и приводит к более плавному изменению шага $$h$$.
    На практике берут те или иные значения $$\\alpha$$,а $$\\alpha^* = 0.9\\, \\alpha$$, чтобы избежать шагов, 
    приводящих к нарушению оценки $$|\\rho_{n+1}| \\leq \\varepsilon$$.
    """)

    st.markdown("""
    При автоматическом выборе шага важным является вопрос, какую точность использовать. На практике чаще всего 
    интересует не абсолютная погрешность в решении, а число верных знаков. Поэтому мерой погрешности обычно считается:
    """)

    st.latex(r"""
    \mu_{n+1} = \begin{cases}
    |\rho_{n+1}| / |y_{n+1}|,\ |y_{n+1}| > \varepsilon, \\
    |\rho_{n+1}|,\ |y_{n+1}| < \varepsilon\tag{37}
    \end{cases}
    """)

    st.markdown("""
    где $$\\rho_{n+1}$$ – локальная погрешность, а $$y_{n+1}$$ – приближённое значение решения, $$e$$ – некоторая 
    заранее заданная константа. Такая мера $$(37)$$ означает, что при больших значениях $$|y_{n+1}|$$ контролируется 
    относительная ошибка, а при малых – абсолютная. В приведённых выше оценках погрешности и формулах для выбора шага
    вместо $$\\varepsilon$$ будет фигурировать:
    """)
    st.latex(r"""
    e_{\mu} = \begin{cases}
    \mu |y|, & |y| > e, \\
    ...
    st.markdown("""
    Для систем уравнений проверка на точность может быть для всех компонент, для некоторых или вообще для одной. При 
    этом контроль может вестись покомпонентно или по норме. В первом случае для каждой компоненты может задаваться своя
    характеристика точности, свои значения $$\\mu, e$$. Удвоение шага $$h$$ делается, если для всех компонент точность
    вычислений превышает заданные значения; деление пополам – если хотя бы для одной компонент контроль точности 
    недостаточен. Контроль по норме означает, что контролируется $$\\|\\rho\\|$$. Часто используются $$(M$$ – число 
    уравнений в системе$$)$$
    """)

    st.latex(r"""
    \|\rho\| = \max_{1 \leq i \leq M} \left|\rho^{(i)}\right|;
        """)
    st.latex(r"""
    \|\rho\| = \sum_{i=1}^M \left|\rho^{(i)}\right|;
    """)
    st.latex(r"""
    \|\rho\| = \left(\sum_{i=1}^M \left|\rho^{(i)}\right|^2 \right)^{1/2}.
    """)

    st.markdown("""
    В этом случае все вычисления при выборе шагов делаются, как и для одного уравнения, только вместо $$|\\rho_{n+1}|$$ 
    используется $$\\|\\rho_{n+1}\\|$$:
    """)
    st.markdown("""
    Из изложенного следует, что методы Рунге–Кутты очень хорошо приспособлены для интегрирования с переменным шагом; при
    смене $$h$$ они не требуют дополнительных преобразований ранее вычисленных значений.
    """)

    st.markdown("""### Реализация метода на python""")
    ...
    def adaptive_integration(func, y0, x0, x_end, h0, epsilon):
        '''Интегрирование с адаптивным выбором шага.'''
        x = x0
        y = y0
        h = h0
        xs = [x]
        ...
        while x < x_end:
            # Пробный шаг полного и половинного размера
            y_full = rk4_step(func, x, y, h)
            y_half_1 = rk4_step(func, x, y, h / 2)
            y_half_2 = rk4_step(func, x + h / 2, y_half_1, h / 2)
    
            # Оценка локальной погрешности
            rho = abs(y_half_2 - y_full)
    
            if rho <= epsilon:
                # Шаг удовлетворяет точности
                x += h
                y = y_half_2
                xs.append(x)
                ys.append(y)
    
                # Увеличиваем шаг, если точность значительно превышает epsilon
                if rho < epsilon / 4:
                    h = min(h * 2, x_end - x)
            else:
                # Шаг слишком большой, уменьшаем его
                h /= 2
    
            # Убедимся, что шаг не выходит за пределы области интегрирования
            if x + h > x_end:
                h = x_end - x
    ...
    def example_function(x, y):
        """Пример функции: y' = -2y + x"""
        return -2 * y + x

    # Streamlit UI
    st.markdown("### Адаптивное интегрирование методом Рунге-Кутты 4-го порядка")

    st.markdown("""
    Для примера возьмем функцию y' = -2y + x
    """)
    # Ввод параметров пользователем
    x0 = st.number_input("Начальное значение x (x0)", value=0.0, step=0.1)
    y0 = st.number_input("Начальное значение y (y0)", value=1.0, step=0.1)
    x_end = st.number_input("Конечное значение x (x_end)", value=5.0, step=0.1)
    h0 = st.number_input("Начальный шаг (h0)", value=0.1, step=0.01)
    epsilon = st.number_input("Допустимая погрешность (epsilon)", value=1e-5, format="%.1e")

    # Кнопка запуска вычислений
    if st.button("Запустить интегрирование"):
        xs, ys = adaptive_integration(example_function, y0, x0, x_end, h0, epsilon)

        # График
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(xs, ys, label="Адаптивное интегрирование", marker="o")
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        ax.set_title("Решение с адаптивным выбором шага")
        ax.legend()
        ax.grid(True)
        ...
        # Таблица результатов
        st.subheader("Результаты интегрирования")
        result_table = {"x": xs, "y": ys}
        st.table(result_table)


# 11.2.4. Устойчивость методов Рунге–Кутты. Недостатки явных методов Рунге–Кутты
def stability_runge_kutta_methods():
    st.header("11.2.4. Устойчивость методов Рунге–Кутты. Недостатки явных методов Рунге–Кутты")

    st.markdown("""
    Рассмотрим задачу Коши:
    """)
    st.latex(r"""
    y' = ay, \quad y(0) = y_0 \tag{38}
    """)
    st.markdown("""
    и применим для её решения явный метод Рунге–Кутты. После исключения $$k_1, k_2, \\dots$$ его можно представить в 
    виде:
    """)
    st.latex(r"""
    y_{n+1} - f(ah)y_n = 0,
    """)
    st.markdown("""
    где $$f(h)$$ – полином некоторой степени от $$h$$. Таким образом, оператор перехода $$R_h = f(ah)$$. Корень 
    характеристического уравнения будет равен:
    """)
    st.latex(r"""
    \lambda = f(ah). \tag{39}
    """)
    ...
    st.markdown("""
    Если $$y_n = y(x_n)$$ – точное значение, то $$y_{n+1}$$ будет отличаться от $$y(x_n + h)$$ на величину 
    $$O(h^{P+1})$$, где $$P$$ – порядок точности схемы Рунге–Кутты. Поскольку:
    """)
    st.latex(r"""
    y(x_n + h) = y(x_n)e^{ah} = y(x_n) \left(1 + ah + \frac{a^2h^2}{2} + \dots \right),
    """)
    st.markdown("""
    а:
    """)
    st.latex(r"""
    y_{n+1} = f(ah)y_n, \tag{40}
    """)
    st.markdown("""
    то:
    """)
    st.latex(r"""
    \lambda \equiv e^{ah} + O(h^{P+1}). 
    """)

    st.markdown("""
    Таким образом, для достаточно малых $$h \\ (h < h_0)$$:
    """)
    st.latex(r"""
    |\lambda| < 1 + ch,
    """)
    st.markdown("""
    и схема Рунге–Кутты будет устойчивой при $$h < h_0$$.

    Как мы уже видели в разделе 10, очень важную роль играет количественная оценка устойчивости. Поэтому рассмотрим 
    несколько явных схем Рунге–Кутты и установим для них оценки шагов на примере задачи $$(38)$$ для $$a < 0$$.

    1. **Метод Эйлера**:
    """)
    st.latex(r"""
    y_{n+1} = y_n + hay_n = (1 + ah)y_n.
    """)
    st.markdown("""
    Таким образом:
    """)
    ...
    """)
    st.markdown("""
    Точное решение $$y(x) = y_0 e^{-|a|x}$$ при $$x \\to \\infty$$ убывает, сохраняя знак. Естественно требовать, чтобы 
    численное решение сохраняло эти свойства. Это приводит к неравенству $$\\ 0 < f(ah) < 1\\ $$ или  $$\\ |a|h < 1.$$
    """)
    ...
    """)
    st.markdown("""
    В случае уравнения $$(38)$$ имеем:
    """)
    st.latex(r"""
    k_1 = hay_n, \quad k_2 = ha\left(y_n + hay_n\right) = ha(1 + ha)y_n.
    """)
    st.markdown("""
    Следовательно:
    """)
    st.latex(r"""
    y_{n+1} = \left[1 + ha + \frac{1}{2}h^2a^2\right]y_n
    """)
    st.markdown("""
    и:
    """)
    st.latex(r"""
    f(ah) = 1 + ha + \frac{1}{2}h^2a^2.
    """)
    st.markdown("""
    Снова, полагая:
    """)
    st.latex(r"""
    0 < 1 + ha + \frac{1}{2}h^2a^2 < 1,
    """)
    st.markdown("""
    определим условие на $$\\ ah$$:
    """)
    st.latex(r"""
    1 + ha + \frac{1}{2}h^2a^2 = \frac{1}{2}\left[(1 + ha)^2 + 1\right].
    """)
    st.markdown("""
    отсюда
    """)
    ...
    st.markdown("""
    Аналогичные неравенства (с разными константами в правой части) будут и для других явных методов Рунге–Кутты, т. е.
    """)
    st.latex(r"""
    |a|h \leq \text{const}
    """)
    st.markdown("""
    является типичным условием, ограничивающим шаги для методов Рунге–Кутты для решения задачи $$(38)$$ при $$a < 0$$.

    Рассмотрим теперь систему линейных уравнений с постоянными коэффициентами:
    """)
    st.latex(r"""
    \begin{cases}
    \overline{y}' = A \overline{y}, \quad A = (a_{ij}), \ i, j = 1, \dots, M,\\
    \overline{y}(0) = \overline{y}_0, \quad \overline{y}_0 = (\overline{y}_0^{(i)}), \ i = 1, 2, \dots, M. \tag{41}
    \end{cases}
    """)

    st.markdown("""
    Применение явных методов типа Рунге–Кутты приведет нас к соотношению:
    """)
    st.latex(r"""
    \overline{y}_{n+1} = f(Ah) \overline{y}_n,
    """)
    st.markdown("""
    где $$f(Ah)$$ – матричный многочлен некоторой степени относительно $$Ah$$.

    Пусть $$A$$ – матрица простой структуры, т. е. пусть она имеет $$M$$ линейно независимых собственных векторов 
    $$\\ \\overline{e}_i$$, 
    соответствующих собственным значениям $$\\lambda_i$$. Тогда общее решение можно записать в виде:
    """)
    st.latex(r"""
    \overline{y}(x) = \sum_{i=1}^M c_i e^{\lambda_i x} \overline{e}_i. \tag{42}
    """)
    st.markdown("""
    В самом деле, разложим $$\\overline{y}_0$$ по собственным векторам:
    """)
    st.latex(r"""
    \overline{y}_0 = \sum_{i=1}^M c_i \overline{e}_i
    """)
    st.markdown("""
    и будем искать $$\\overline{y}(x)$$ в виде $$\\ \\overline{y} = \\sum_{i=1}^M f_i(x) \\overline{e}_i.$$
    Тогда из $$(41)$$ находим:
    """)
    st.latex(r"""
    \sum_{i=1}^M f_i'(x) \overline{e}_i = \sum_{i=1}^M f_i(x) \lambda_i \overline{e}_i.
    """)
    st.markdown("""
    В силу линейной независимости $$\\overline{e}_i$$ имеем:
    """)
    st.latex(r"""
    f_i' = \lambda_i f_i(x), \quad f_i = c_i e^{\lambda_i x}.
    """)
    st.markdown("""
    Обозначим $$c_i e^{\\lambda_i x_n}$$ через $$c_n^i$$, тогда:
    """)
    st.latex(r"""
    \overline{y}(x_n) = \sum_{i=1}^M c_n^i \overline{e}_i.
    """)
    ...
    """)
    st.latex(r"""
    \overline{y}_{n+1} = f(Ah)\overline{y}_n = \sum_{i=1}^M f(Ah)c_n^i\overline{e}_i = \sum_{i=1}^M f(\lambda_i h)c_n^i
    \overline{e}_i = \sum_{i=1}^M c_{n+1}^i\overline{e}_i.
    """)
    st.markdown("""
    Таким образом, все составляющие решения $$(42)$$ при численном счёте преобразуются независимо, и:
    """)
    st.latex(r"""
    c_{n+1}^i = f(\lambda_i h)c_n^i.
    """)
    st.markdown("""
    Эта формула для пересчёта коэффициентов совпадает с $$(40)$$ для одного уравнения.

    Итак, для системы уравнений ограничение на шаг $$h$$ в явных методах Рунге–Кутты имеет вид (для $$\\lambda_i < 0$$):
    """)
    st.latex(r"""
    |\lambda_i| h \leq \text{const}.
    """)

    st.markdown("""
    Если среди собственных чисел матрицы $$A$$ есть $$\\lambda_i < 0, |\\lambda_i| \\gg 1$$, то решение такой системы 
    потребует очень малого шага $$h$$. Существенно, что этот шаг не может быть увеличен, даже если быстро меняющаяся 
    компонента:
    """)
    st.latex(r"""
    c_i e^{+\lambda_i x} \overline{e}_i (\lambda_i < 0, |\lambda_i| \gg 1)
    """)
    st.markdown("""
    затухнет (т. е. станет ничтожно малой по сравнению с другими компонентами). 
    Поэтому использование явных методов Рунге–Кутты для таких задач приводит к огромному объёму вычислений, что делает 
    их непригодными для этих задач.
    """)
