...
def free_nodes_gaussian_quadratures():
    ...
    \dots, x^{2n-1}$$. Покажем, как в этом случае можно получить узлы и веса. Будем считать, что функция $$\\rho(x) > 0$$
     и непрерывна на $$(\\alpha, \\beta)$$, и интеграл
    """)
    ...
    $$(\\rho(x)\\equiv 1)$$
    или Гаусса–Кристоффеля $$(\\rho(x)\\ne 1)$$ являются нулями соответствующего многочлена $$P_n(x)$$ из системы многочленов, 
    ортогональных на $$(\\alpha, \\beta)$$ с весом $$\\rho(x)$$.
    ...
    st.markdown("""
    (это те же самые многочлены, которые фигурируют в методе интерполяции по Лагранжу). Степень их равна $$n - 1$$, следовательно, формула будет точной для каждого $$L_m$$ точка
    """)
    ...
    st.markdown("""
    Пусть $$t_1, \dots, t_n$$ — нули многочлена Лежандра $$P_n(t)$$, $$\\tilde{W}_1,\\ldots,\\tilde{W}_n$$ — соответствующие веса 
     (для $$[-1, 1]$$), тогда
    """)
    ...
    st.latex(r"""
    \int_{\alpha}^{\beta} y \, dx = \frac{\beta - \alpha}{2} \left[ y\left(\alpha + \frac{3 - \sqrt{3}}{6} (\beta - \alpha)\right) + y\left(\alpha + \frac{3 + \sqrt{3}}{6} (\beta - \alpha)\right)\right].
    """)
...
def improving_the_accuracy_of_quadrature_formulas():
    ...
    st.markdown("""
        Видно, что уточнение первого столбца методом Эйткена дает хороший результат. Вычислим порядок $$ p $$ формулы трапеций для этой функции:
        """)
    ...
    st.markdown("""
        Т.е. порядок – дробное число! Это – обычный факт для квадратурных формул, применяемых к функциям с особенностью, которые явно её не учитывают. Если особенности нет, то теоретический порядок должен быть при $$ h \\to 0 $$ близким к вычисленному.
        """)
...
def irregular_functions():
    st.header(""" Нестандартные формулы""")
