...
def simple_iterecia():
    ...
    def simple_iteration_method(f, x0, y0, x_end, h, tol=1e-6, max_iter=100):
        """
        Метод простой итерации для решения уравнения y' = f(x, y) с начальным условием y(x0) = y0.

        Параметры:
        f        — функция, задающая правую часть уравнения f(x, y).
        x0, y0   — начальные значения x и y.
        x_end    — конечное значение x, до которого нужно решить уравнение.
        h        — шаг по x.
        tol      — точность для итераций по y (по умолчанию 1e-6).
        max_iter — максимальное количество итераций для нахождения y_{n+1} (по умолчанию 100).

        Возвращает:
        Список значений x и соответствующие значения y.
        """
        x_values = [x0]
        y_values = [y0]
        ...
        while x < x_end:
            # Переходим на следующий шаг по x
            x += h
            x_values.append(x)

            # Начальное приближение для y_{n+1}
            y_new = y

            for i in range(max_iter):
                # Используем формулу y_{n+1} = y_n + h * f(x_n, y_n)
                y_iter = y + h * f(x, y_new)

                # Проверяем достижение заданной точности
                if abs(y_iter - y_new) < tol:
                    y_new = y_iter
                    break
    ...
    st.title("Метод простой итерации для решения ОДУ")

    # Ввод уравнения
    equation_input = st.text_input("Введите правую часть уравнения y' = f(x, y):", value="x + y")
    try:
    ...
    x0 = st.number_input("Начальное значение x:", value=0.0, step=0.1)
    y0 = st.number_input("Начальное значение y:", value=1.0, step=0.1)
    x_end = st.number_input("Конечное значение x:", value=2.0, step=0.1)
    h = st.number_input("Шаг по x:", value=0.1, step=0.01, format="%.2f")
    ...
    if st.button("Рассчитать"):
        try:
            ...
            x_vals, y_vals = simple_iteration_method(f, x0, y0, x_end, h, tol, max_iter)

            # Округление значений и отображение таблицы
            df = pd.DataFrame({'x': [round(val, 2) for val in x_vals], 'y': [round(val, 2) for val in y_vals]})
            st.write("Таблица значений x и y:")
            st.dataframe(df)

            # Построение графика
            fig, ax = plt.subplots()
            ax.plot(x_vals, y_vals, marker='o', label='Численное решение')
            ax.set_xlabel('x')
            ax.set_ylabel('y')
            ax.set_title("Решение ОДУ методом простой итерации")
            ax.grid(True)
            ax.legend()
